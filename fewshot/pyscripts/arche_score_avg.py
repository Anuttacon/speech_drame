...
def extract_scores_from_response(response: str) -> Dict[str, int]:
    ...
    try:
        ...
        # Extract the evaluation dimensions
        scores = {}
        dimensions = [
            'audio_quality', 'human_likeness', 'appropriateness', 'content_pass',
        ]
        
        for dim in dimensions:
            if dim in parsed:
                if dim == 'content_pass':
                    # content_pass is boolean, convert to float (0.0 or 1.0)
                    scores[dim] = float(parsed[dim])
                else:
                    scores[dim] = float(parsed[dim])
            else:
                if dim == 'content_pass':
                    scores[dim] = 1.0  # Default to pass if missing
                else:
        ...
        return {
            'audio_quality': 1, 'human_likeness': 1, 'appropriateness': 1, 'content_pass': 1.0,
        }
...
def extract_ground_truth_scores(annotation_data: Dict[str, Any]) -> Dict[str, List[int]]:
    ...
    def safe_int_convert(value):
        ...
        if isinstance(value, str):
            ...
            return 1 if value else 0  # Convert boolean to int (True=1, False=0)
        return int(value)
    ...
    source_dimensions = ["audio_quality", "human_likeness", "appropriateness", "content_pass"]
...
def calculate_content_pass_accuracy(predictions: List[float], ground_truth: List[int]) -> Dict[str, float]:
    """Calculate content pass accuracy between predictions and ground truth"""
    if len(predictions) != len(ground_truth) or len(predictions) == 0:
        return {
    ...
    try:
        ...
        # Calculate majority voting for ground truth (fail if >50% fail)
        gt_binary = []
        for gt_list in ground_truth:
            if isinstance(gt_list, list):
                # Count fails (0s) vs passes (1s)
                fail_count = sum(1 for x in gt_list if x == 0)
                pass_count = sum(1 for x in gt_list if x == 1)
                # If more than half are fails, set as fail (0), otherwise pass (1)
                gt_binary.append(0 if fail_count > len(gt_list) / 2 else 1)
            else:
                # Single value, convert to binary
                gt_binary.append(1 if gt_list >= 0.5 else 0)
        ...
        print(f"Error calculating content pass accuracy: {e}")
        return {
            'accuracy': 0.0, 'precision': 0.0, 'recall': 0.0, 'f1': 0.0,
            'true_positives': 0, 'false_positives': 0, 'true_negatives': 0, 'false_negatives': 0
        }
...
def analyze_results(avg_scores: Dict[str, Dict[str, float]], ground_truth_path: str) -> Dict[str, Any]:
    ...
    for dim in dimensions:
        ...
        for file_id, scores in avg_scores.items():
            if file_id in ground_truth:
                ...
                try:
                    ...
                    if dim == 'content_pass':
                        # For content_pass, keep the raw list for majority voting
                        ground_truth_scores.append(gt_scores[dim])
                    else:
        ...
        # Calculate metrics
        if dim == 'content_pass':
            # Use content pass accuracy calculation
            metrics = calculate_content_pass_accuracy(predictions, ground_truth_scores)
        else:
            # Use regular metrics calculation
    ...
    results['trial_consistency'] = {
        'avg_trial_count': np.mean([scores.get('audio_quality_count', 0) for scores in avg_scores.values()]),
        'std_trial_count': np.std([scores.get('audio_quality_count', 0) for scores in avg_scores.values()]),
        'avg_std_across_dimensions': np.mean([results['dimensions'][dim]['trial_std_mean'] for dim in dimensions if dim != 'content_pass'])
    }
...
def create_visualizations(results: Dict[str, Any], output_dir: str):
    ...
    non_content_pass_dims = [dim for dim in dimensions if dim != 'content_pass']
    
    # 1. Correlation heatmap (excluding content_pass)
    if non_content_pass_dims:
        plt.figure(figsize=(12, 8))
        correlation_data = []
        for dim in non_content_pass_dims:
    ...
    # 2. Error metrics comparison (excluding content_pass)
    if non_content_pass_dims:
        plt.figure(figsize=(15, 5))
        
        mae_values = [results['dimensions'][dim]['mae'] for dim in non_content_pass_dims]
        rmse_values = [results['dimensions'][dim]['rmse'] for dim in non_content_pass_dims]
        
        x = np.arange(len(non_content_pass_dims))
        width = 0.35
        ...
        plt.xticks(x, non_content_pass_dims, rotation=45, ha='right')
        plt.legend()
        plt.grid(True, alpha=0.3)
        
        plt.subplot(1, 2, 2)
        mape_values = [results['dimensions'][dim]['mape'] for dim in non_content_pass_dims]
        plt.bar(non_content_pass_dims, mape_values, alpha=0.8, color='orange')
        plt.xlabel('Dimensions')
        plt.ylabel('MAPE (%)')
    ...
    # 3. Trial consistency analysis (excluding content_pass)
    if non_content_pass_dims:
        plt.figure(figsize=(12, 5))
        
        trial_stds = [results['dimensions'][dim]['trial_std_mean'] for dim in non_content_pass_dims]
        trial_counts = [results['dimensions'][dim]['trial_count_mean'] for dim in non_content_pass_dims]
        
        plt.subplot(1, 2, 1)
        plt.bar(non_content_pass_dims, trial_stds, alpha=0.8, color='green')
        plt.xlabel('Dimensions')
        ...
        plt.subplot(1, 2, 2)
        plt.bar(non_content_pass_dims, trial_counts, alpha=0.8, color='purple')
        plt.xlabel('Dimensions')
    ...
    # 4. Content Pass Accuracy visualization
    if 'content_pass' in results['dimensions']:
        plt.figure(figsize=(10, 6))
        
        cp_metrics = results['dimensions']['content_pass']
        metrics_names = ['Accuracy', 'Precision', 'Recall', 'F1']
        metrics_values = [cp_metrics['accuracy'], cp_metrics['precision'], cp_metrics['recall'], cp_metrics['f1']]
        
        bars = plt.bar(metrics_names, metrics_values, alpha=0.8, color=['blue', 'green', 'orange', 'red'])
        plt.xlabel('Metrics')
        plt.ylabel('Score')
        plt.title('Content Pass Classification Metrics')
        plt.ylim(0, 1)
        plt.grid(True, alpha=0.3)
        ...
        plt.tight_layout()
        plt.savefig(output_path / 'content_pass_accuracy.png', dpi=300, bbox_inches='tight')
        plt.close()
...
def print_detailed_results(results: Dict[str, Any]):
    ...
    for dim, metrics in results['dimensions'].items():
        if dim == 'content_pass':
            # Special handling for content_pass
            print(f"{dim:<25} {'N/A':<10} {'N/A':<10} {'N/A':<8} {'N/A':<8} {'N/A':<8} {'N/A':<8} {'N/A':<10}")
        else:
    ...
    # Content Pass Accuracy Results
    if 'content_pass' in results['dimensions']:
        cp_metrics = results['dimensions']['content_pass']
        print(f"\nCONTENT PASS ACCURACY METRICS:")
        print(f"  Accuracy: {cp_metrics['accuracy']:.4f}")
        print(f"  Precision: {cp_metrics['precision']:.4f}")
        print(f"  Recall: {cp_metrics['recall']:.4f}")
        print(f"  F1 Score: {cp_metrics['f1']:.4f}")
        print(f"  True Positives: {cp_metrics['true_positives']}")
        print(f"  False Positives: {cp_metrics['false_positives']}")
        print(f"  True Negatives: {cp_metrics['true_negatives']}")
        print(f"  False Negatives: {cp_metrics['false_negatives']}")
    
    # Summary statistics (excluding content_pass)
    non_cp_dimensions = {k: v for k, v in results['dimensions'].items() if k != 'content_pass'}
    correlations = [metrics['correlation'] for metrics in non_cp_dimensions.values()]
    spearmans = [metrics['spearman'] for metrics in non_cp_dimensions.values()]
    ktaus = [metrics['ktau'] for metrics in non_cp_dimensions.values()]
    maes = [metrics['mae'] for metrics in non_cp_dimensions.values()]
    rmses = [metrics['rmse'] for metrics in non_cp_dimensions.values()]
    mapes = [metrics['mape'] for metrics in non_cp_dimensions.values()]
