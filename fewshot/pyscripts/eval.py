...
from utils import str2bool, load_few_shot_examples, select_few_shot_examples
...
def get_parser():
    ...
    parser.add_argument("--prompt_type", type=str, choices=["separate", "combined", "archetype_combined", "archetype_separate", "few_shot_combined", "archetype_few_shot_combined"], required=True, default="separate")
    ...
    # Few-shot arguments
    parser.add_argument("--few_shot_examples_path", type=str, default=None, 
                       help="Path to JSONL file containing few-shot examples")
    parser.add_argument("--num_few_shot", type=int, default=3, 
                       help="Number of few-shot examples to use")
    parser.add_argument("--few_shot_strategy", type=str, choices=["random", "similar", "diverse"], 
                       default="random", help="Strategy for selecting few-shot examples")
...
def main():
    ...
    if args.few_shot_examples_path:
        few_shot_examples = load_few_shot_examples(args.few_shot_examples_path, args.prompt_type)
    ...
    for batch_idx in tqdm(range(0, total_batches), desc="Processing batches"):
        ...
        elif args.prompt_type == "few_shot_combined" and few_shot_examples is not None:
            # Few-shot evaluation for combined prompts
            batch_results = []
            for item in batch:
                # Select few-shot examples for this item
                ...
                # Use few-shot evaluation with conversation format if available
                if hasattr(model, 'evaluate_few_shot_with_schema'):
                    result = model.evaluate_few_shot_with_schema(selected_examples, item, rubric_prompt)
                else:
                    # Fallback: use regular evaluation without few-shot examples
                    user_prompt, speech_path = get_user_prompt(item)
                    full_prompt = rubric_prompt + "\n" + user_prompt
                    
                    if args.use_schema and hasattr(model, 'evaluate_with_schema'):
                        result = model.evaluate_with_schema(speech_path, full_prompt)
                    else:
                        result = model.generate(full_prompt, speech_path)
                
                batch_results.append(result)
            
            results.extend(batch_results)
        
        elif args.prompt_type == "archetype_few_shot_combined" and few_shot_examples is not None:
            # Few-shot evaluation for archetype prompts
            batch_results = []
            for item in batch:
                # Select few-shot examples for this item
                selected_examples = select_few_shot_examples(
                ...
                # Use few-shot evaluation with conversation format if available
                fewshot_success = False
                scheme_success = False
                generate_success = False
                if hasattr(model, 'evaluate_few_shot_with_schema'):
                    try:
                        result = model.evaluate_few_shot_with_schema(selected_examples, item, rubric_prompt, key="archetype")
                        fewshot_success = True
                    except Exception as e:
                        print(f"Error in few-shot evaluation: {str(e)}")
                if not fewshot_success:
                    # Fallback: use regular evaluation without few-shot examples
                    user_prompt, speech_path = get_archetype_user_prompt(item)
                    full_prompt = rubric_prompt + "\n" + user_prompt
                    
                    try:
                        result = model.evaluate_with_schema(speech_path, full_prompt, key="archetype")
                        scheme_success = True
                    except Exception as e:
                        print(f"Error in schema evaluation: {str(e)}")

                    if not scheme_success:
                        try:
                            result = model.generate(full_prompt, speech_path)
