...
from dataset.archetype_prompt import (
    detailed_archetype_dimension_prompts,
    general_archetype_prompt,
    general_archetype_prompt_detailed,
    archetype_caption_prompt,
    archetype_emotion_prompt
)
from dataset.dataset import get_archetype_prompt
...
def _create_role_evaluation_prompt(data_item: Dict[str, Any], eval_type: str, specific_dimension: Optional[str] = None, use_gt_thinking: Optional[bool] = False) -> tuple[str, str, str]:
    ...
    if eval_type == "specific_dimension" and specific_dimension:
        if specific_dimension not in detailed_archetype_dimension_prompts:
            raise ValueError(f"Unknown dimension: {specific_dimension}")
        
        dataset_name = data_item.get("dataset_name", "Arche")
        
        if dataset_name == "Arche":
            # Get the dimension prompt exactly as in dataset.py
            dimension_prompt = detailed_archetype_dimension_prompts[specific_dimension]
            
            # Get user prompt exactly as in dataset.py
            user_prompt, speech_path = get_archetype_prompt(data_item)
    ...
    #     general_prompt_base = general_archetype_prompt_detailed if data_item["dataset_name"] == "RoleAllFixOrder" else general_archetype_prompt
...
def _get_score_from_token_probabilities(model, inputs, processor, data_args, key="1-5" or "0-1") -> tuple:
    """
    ...
    """
    available_scores = ["1", "2", "3", "4", "5"] if key == "1-5" else ["0", "1"]
    ...
    force_words_ids = [
        [processor.tokenizer.encode(score, add_special_tokens=False)[0] for score in available_scores],
                ...
                for step_idx, step_logits in enumerate(sample_scores):
                    if step_idx == score_token_pos:
                        ...
                        if key == "1-5":
                            ...
                            expected_score = torch.dot(score_probs, torch.arange(0, 2, device=logits.device).float())
...
def main():
    ...
    for i in tqdm(range(0, len(datas), batch_size), desc="Processing batches"):
        ...
        batch_probability_scores, batch_responses = _get_score_from_token_probabilities(audio_model, inputs, audio_processor, data_args, key="0-1" if data_args.evaluation_type == "specific_dimension" and data_args.specific_dimension == "content_pass" else "1-5")
    ...
    for idx, (input_example, model_output) in enumerate(zip(datas, all_outputs)):
        ...
        try: 
            if "audio_quality" not in input_example:
                input_example = input_example["annotations"] 
            # Add ground truth ratings
            result["ground_truth_ratings"] = {
                "content_pass": input_example.get("content_pass", []),
                "audio_quality": input_example.get("audio_quality", []),
                "human_likeness": input_example.get("human_likeness", []),
                "appropriateness": input_example.get("appropriateness", [])
            }
        except Exception as e:
